              ...
              <div className="flex items-end justify-between -mt-10 mb-4 flex-wrap gap-3">
                ...
                <div className="flex gap-2 pb-1">
                  <button
                    ...
                    className={cn('p-2.5 rounded-xl border transition-all', liked ? 'bg-primary-50 border-primary-200 text-primary-500' : 'bg-slate-100 border-transparent hover:bg-slate-200 text-slate-500')}
                  >
                    <Heart className={cn('w-4 h-4', liked && 'fill-primary-500')} />
                  </button>
            ...
            <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-soft">
              ...
              <div className="grid grid-cols-4 sm:grid-cols-7 gap-2 mb-6">
                {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((d, i) => (
                  <div key={d} className={cn('text-center p-3 rounded-xl', [0,2,4,5].includes(i) ? 'bg-primary-50 border border-primary-200' : 'bg-slate-50 border border-slate-100')}>
                    <p className="text-xs font-medium text-slate-500">{d}</p>
                    <p className={cn('text-sm font-bold mt-1', [0,2,4,5].includes(i) ? 'text-primary-700' : 'text-slate-300')}>
                      {[0,2,4,5].includes(i) ? '✓' : '—'}
                    </p>
                  </div>
                ))}
