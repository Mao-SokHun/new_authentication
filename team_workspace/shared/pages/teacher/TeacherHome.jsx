...
const sessionColumns = [
  ...
  },
  {
    ...
    className: 'text-center',
    render: (row) => (
      <span className={clsx('inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium', STATUS_STYLES[row.status])}>
        {row.status}
      </span>
    ),
  },
    ...
    <PageAmbient variant="teacher" className="space-y-5">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-slate-800 via-slate-800 to-slate-900 text-white px-5 py-4">
        <div className="relative z-10">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">{t('teacherDash.label')}</p>
          <h1 className="text-xl font-bold mt-0.5">{t('teacherDash.heading')}</h1>
          <p className="text-sm text-slate-300 mt-1">{t('teacherDash.subtitle')}</p>
        </div>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatMetric label={t('teacherDash.totalSessions')} value="2,684" tone="primary" />
        <StatMetric label={t('teacherDash.activeStudents')} value="88" tone="success" />
        <StatMetric label={t('teacherDash.avgRating')} value="4.9 ★" tone="warning" />
        <StatMetric label={t('teacherDash.thisWeek')} value="+12%" change={`+12% ${t('teacherDash.vsLastWeek')}`} tone="info" />
      </div>
      ...
      <PageCard padding={false} className="overflow-hidden">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-5 pb-3">
          <h2 className="font-bold text-slate-800 text-base">
            {t('teacherDash.userActivity')} ({periodLabels[period]})
          </h2>
        ...
        <div className="flex flex-col lg:flex-row gap-0 divide-y lg:divide-y-0 lg:divide-x divide-slate-100">
          ...
          <div className="lg:w-56 p-5">
            <h3 className="text-sm font-bold text-slate-800 mb-4">{t('teacherDash.activityBySubject')}</h3>
      ...
      <div>
        <h2 className="font-bold text-slate-800 text-base mb-3 px-1">{t('teacherDash.sessions')}</h2>
        <DataTable
