            ...
            <div className="grid sm:grid-cols-2 gap-3">
              {helpCategories.map((c) => (
                <PageCard key={c.label} className="p-4">
                  <p className="text-sm font-bold text-slate-800">{c.labelKm}</p>
                  <p className="text-xs font-semibold text-slate-500">{c.label}</p>
                  <p className="text-xs text-slate-600 mt-1.5">{c.descriptionKm}</p>
              ...
              <div className="divide-y divide-slate-100">
                {helpFaqs.map((faq, i) => (
                  <div key={faq.q}>
                    ...
                    >
                      <div>
                        <p className="text-sm font-bold text-slate-800">{faq.qKm}</p>
                        <p className="text-xs font-medium text-slate-500 mt-0.5">{faq.q}</p>
                      </div>
                      <ChevronDown
                        className={clsx(
                    ...
                    {openFaq === i && (
                      <div className="px-5 pb-4 space-y-1.5">
                        <p className="text-sm text-slate-700 leading-relaxed">{faq.aKm}</p>
