import { FILTER_ALL, majorOptions, subjectOptions } from '../filters/teacherFilters'
...
function categorizeMajor(major) {
  ...
  if (/^language$/.test(m)) return 'Language'
  if (/education|law|medicine/.test(m)) return 'Education'
  return 'Other'
}
...
function categorizeSubject(subject) {
  ...
  if (/english|khmer|chinese|french|korean|japanese|vietnamese|spanish|german/.test(s)) return 'Language'
  return 'STEM'
}
