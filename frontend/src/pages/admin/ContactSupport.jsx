...
const channels = [
  ...
  { icon: MessageSquare, label: 'Live Chat', sub: 'Chat with an agent', color: 'bg-emerald-100 text-emerald-600', time: 'Now online' },
  ...
  if (submitted) {
    return (
      <PageCard className="max-w-sm mx-auto text-center py-12">
        ...
        <p className="text-slate-500 text-sm mb-6">Our support team will get back to you within 24 hours.</p>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <PageCard className="lg:col-span-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-4">
              <Input label="Your Name" placeholder="Admin User" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required />
              <Input label="Email Address" type="email" placeholder="admin@rokkru.com" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required />
            </div>
        ...
        <div className="space-y-4">
          <PageCard padding={false} className="overflow-hidden">
            <img src="https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=400&h=200&fit=crop" alt="Support team" className="w-full h-36 object-cover" />
            <div className="p-4">
              <h3 className="font-bold text-slate-800 text-sm">Our Support Team</h3>
              <p className="text-xs text-slate-400 mt-1">Dedicated specialists ready to help you resolve platform issues quickly.</p>
            </div>
          </PageCard>
          <PageCard>
            <h3 className="font-bold text-slate-800 text-sm mb-3">Common Topics</h3>
            <ul className="space-y-1">
              {['Account access issues', 'Teacher verification', 'Payment disputes', 'Platform bugs', 'API integration'].map((t) => (
                <li key={t}>
                  <button type="button" className="flex items-center justify-between w-full py-2 text-xs text-slate-600 hover:text-primary-600 transition-colors">
                    <span>{t}</span>
